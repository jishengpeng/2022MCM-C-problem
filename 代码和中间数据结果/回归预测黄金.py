...
sheet = book.add_sheet("回归预测黄金", cell_overwrite_ok=True)
col = ("日期","标签","真实值","预测值","误差")  # 元组,如果需要院校简介另加
for i in range(0, 5):
    sheet.write(0, i, col[i])
sheet.write(1,0,"9/11/16")
sheet.write(1,1,1)
# sheet.write(1,2,1324.6)
# sheet.write(1,3,1324.6)
# sheet.write(1,4,0)
#
...
for i in range(0, 1824):
    ...
    if(df.values[i+2][2]==1):
        ...
        sheet.write(i + 3, 1, df.values[i + 2][2])
    else:
        sheet.write(i + 3, 0, df.values[i+2][0])
        sheet.write(i + 3, 1, df.values[i + 2][2])
        sheet.write(i + 3, 2, ji2[i])
        sheet.write(i + 3, 3, ji1[i+2][0][0][0])
        sheet.write(i + 3, 4, abs(ji1[i + 2][0][0][0]-ji2[i]))

book.save("回归预测黄金.xls")
